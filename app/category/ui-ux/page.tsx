...
export default function UIUXPage() {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        const data = await projectsAPI.getAll("ui-ux")
        setProjects(data)
      } catch (error) {
        ...
        <div className="grid h-full grid-cols-1 gap-4 lg:grid-cols-[420px_1fr]">
            ...
            >
              ...
              <div>
                ...
                <p className="mt-4 max-w-[42ch] text-lg text-neutral-600">
                  User-centered design that creates intuitive, delightful digital experiences. We design interfaces that
                  users love and that drive real business results.
                </p>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="flex items-center justify-center py-20">
                <p className="text-neutral-600">No UI/UX projects available yet.</p>
              </div>
            ) : (
              projects.map((p, idx) => (
                <ProjectCard
